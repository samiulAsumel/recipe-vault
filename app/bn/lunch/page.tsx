...
export const metadata: Metadata = buildPageMetadata({
  title: dict.mealHubs.lunch.title,
  description: dict.mealHubs.lunch.description,
  path: "/bn/lunch/",
});

export default async function LunchPageBn(): Promise<React.JSX.Element> {
  ...
  return (
    <main className="mx-auto flex max-w-6xl flex-col gap-8 px-6 py-16">
      <header className="flex flex-col gap-4">
        <h1 className="font-display text-5xl text-ink">{dict.mealHubs.lunch.title}</h1>
        <AtlasRule />
      </header>

      <Suspense fallback={<div className="h-32" aria-hidden />}>
        <FilteredDishes dishes={dishes} emptyMessage={dict.mealHubs.lunch.emptyMessage} hideMealTime />
      </Suspense>
    </main>
  );
}
