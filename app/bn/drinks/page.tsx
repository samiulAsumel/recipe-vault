...
export const metadata: Metadata = buildPageMetadata({
  title: dict.mealHubs.drinks.title,
  description: dict.mealHubs.drinks.description,
  path: "/bn/drinks/",
});

export default async function DrinksPageBn(): Promise<React.JSX.Element> {
  ...
  return (
    <main className="mx-auto flex max-w-6xl flex-col gap-8 px-6 py-16">
      <header className="flex flex-col gap-4">
        <h1 className="font-display text-5xl text-ink">{dict.mealHubs.drinks.title}</h1>
        <AtlasRule />
      </header>

      <Suspense fallback={<div className="h-32" aria-hidden />}>
        <FilteredDishes dishes={dishes} emptyMessage={dict.mealHubs.drinks.emptyMessage} hideMealTime />
      </Suspense>
    </main>
  );
}
